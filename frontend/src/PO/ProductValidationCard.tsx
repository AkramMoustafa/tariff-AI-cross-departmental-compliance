...
const ProductValidationCard = ({ lineItems, onUpdate }: Props) => {
  ...
  const [improvingIndexes, setImprovingIndexes] = useState<Set<number>>(new Set());
  const [classifyingIndexes, setClassifyingIndexes] = useState<Set<number>>(new Set());

  const getDescriptionConfidence = (desc: string): "LOW" | "MEDIUM" | "HIGH" => {
  ...
  useEffect(() => {
    if (lineItems.length > 0) {
      setTimeout(() => {
        document.getElementById("product-validation")?.scrollIntoView({ behavior: "smooth" });
      }, 200);
    }
  }, [lineItems]);
    ...
    setItems(prev =>
      lineItems.map((item, index) => {
        ...
        return {
          ...
          improvedDescription:
            existing?.improvedDescription || item.normalizedDescription || item.description || "",
          isCollapsed: existing?.isCollapsed || false,
          quantity: item.quantity || "",
          unitPrice: item.unitPrice || "",
          suggestedHSCode: existing?.suggestedHSCode || item.hsCode || "",
          suggestedHSDescription: existing?.suggestedHSDescription || "",
  ...
  useEffect(() => {
    ...
    onUpdate(
      items.map(item => ({
        ...
        normalizedDescription: item.improvedDescription,
        quantity: item.quantity,
        unitPrice: item.unitPrice,
  ...
  const handleConfirm = (index: number) => {
    setItems(prev =>
      prev.map((item, i) => (i !== index ? item : { ...item, isCollapsed: true }))
    );
  };

  const handleImproveDescription = async (index: number) => {
    const item = items[index];
    const sourceDescription = item.suggestedHSDescription || item.improvedDescription;
    if (!sourceDescription.trim()) {
      alert("No description available");
      return;
    }
    setImprovingIndexes(prev => new Set(prev).add(index));
    try {
      const res = await improveDescription1({ description: sourceDescription });
      setItems(prev =>
        prev.map((it, i) =>
          i !== index
            ? it
            : {
                ...it,
                improvedDescription: res.description,
  ...
  const handleClassifyHS = async (index: number) => {
    ...
    setClassifyingIndexes(prev => new Set(prev).add(index));
    try {
      ...
      setItems(prev =>
        prev.map((it, i) => {
          ...
          return {
            ...
            suggestedHSDescription: res.suggested?.description || res.hs_description || "",
            hsConfidence: (res.confidence || "LOW").toUpperCase() as "LOW" | "MEDIUM" | "HIGH",
            confirmedHSCode:
  ...
  const handleFieldChange = (index: number, field: keyof EditableLineItem, value: string) => {
    setItems(prev =>
      prev.map((item, i) => {
    ...
    >
      <Box sx={{ mb: 3 }}>
        ...
        <Typography sx={{ fontSize: 13, color: "#64748b", mt: 0.5 }}>
          Improve product descriptions, run AI classification, and confirm HS codes before tariff analysis.
        </Typography>
      </Box>

      <Stack spacing={3}>
        {items.map((item, index) => {
          const isImproving = improvingIndexes.has(index);
          const isClassifying = classifyingIndexes.has(index);
          const isItemBusy = isImproving || isClassifying;
            ...
            <Box key={index}>
              ...
              >
                ...
                <Button
                  ...
                  onClick={() =>
                    setItems(prev =>
                      prev.map((it, i) => (i === index ? { ...it, isCollapsed: false } : it))
                    )
                  }
                >
                  Edit
              ...
              >
                ...
                <Stack direction={{ xs: "column", sm: "row" }} spacing={1.5} sx={{ mb: 0.5 }}>
                  ...
                  >
                    {isImproving ? (
                      <>
                        <CircularProgress size={14} sx={{ mr: 1, color: "#64748b" }} />
                        Improving...
                      </>
                    ) : (
                      "✨ Improve Description"
              ...
              <Grid container spacing={2.5} alignItems="stretch">
                <Grid item xs={12} md={6}>
                  ...
                  >
                    ...
                    <Typography sx={{ fontSize: 16, fontWeight: 500, color: "#0f172a", lineHeight: 1.5 }}>
                ...
                <Grid item xs={12} md={6}>
                  <TextField
                    ...
                    onChange={(e) => handleFieldChange(index, "improvedDescription", e.target.value)}
                    sx={{
                      ...inputSx,
                      "& .MuiOutlinedInput-root": {
                ...
                <Grid item xs={12} md={6}>
                  <TextField
                    ...
                    onChange={(e) => handleFieldChange(index, "confirmedHSCode", e.target.value)}
                    sx={inputSx}
                  ...
                  <Grid item xs={12}>
                    <Box sx={{ mt: 1 }}>
                      ...
                      <Stack direction="row" spacing={1} flexWrap="wrap">
                        <Chip
                          label={item.confirmedHSCode ? "✅ Verified" : "⚠️ Needs Review"}
                          color={item.confirmedHSCode ? "success" : "warning"}
                          size="small"
                        />
                        {item.alternatives.map((alt, idx) => {
                          const isSelected = item.confirmedHSCode === alt.code;
                          return (
                            <Chip
                              key={idx}
                              label={alt.code}
                              ...
                              onClick={() => handleFieldChange(index, "confirmedHSCode", alt.code)}
                              sx={{ cursor: "pointer" }}
                            />
                          );
                        })}
                ...
                {(item.descriptionConfidence === "LOW" || item.hsConfidence === "LOW") && (
                  <Grid item xs={12}>
                    <Box
                      sx={{
                        mt: 0.5,
                    ...
                    >
                      <Typography sx={{ fontSize: 12, color: "#b91c1c", fontWeight: 600 }}>
                        Warning: The description may be too vague for reliable HS classification.
                        Add more detail before continuing.
                      </Typography>
                    </Box>
                  </Grid>
                )}
                ...
                <Grid item xs={12}>
                  <Box sx={{ display: "flex", justifyContent: "flex-end", mt: 1 }}>
                    <Button
                      ...
                      disabled={!item.confirmedHSCode || isItemBusy}
                    >
